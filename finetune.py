...
def main():    
    # Load dataset
    dataset = Dataset() 
    loaded_img_train = np.load("Computer_Vision/image_array.npy")
    loaded_lab_train = np.load("label_array.npy")
    ...
    #"""
    with open('Computer_Vision/image_array.npy', 'rb') as f:
        junk, header_len = struct.unpack('<8sh', f.read(10))
        f.close()
    # loaded_img_train= np.memmap('Computer_Vision/image_array.npy',dtype='float64',shape=(575, 227, 227, 3),offset=6+2+2+header_len)


    with open('label_array.npy', 'rb') as f:
        junk, header_len = struct.unpack('<8sh', f.read(10))
        f.close()
    # loaded_lab_train= np.memmap('label_array.npy',dtype='float64',shape=(575, 7),offset=6+2+2+header_len)
